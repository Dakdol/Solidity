...
struct Student{
     ...
     mapping(uint => Student) number_Stu;
     mapping(string => Student) name_Stu;

     Student[] Students;

    //* 학생 추가 기능 - 특정 학생의 정보를 추가
    function setStudent(string memory _name,uint _num, string memory _class, uint _score,uint _grade) public {
        
        Students.push(Student(_name,_num,_class,_score,_grade));
    }

    //* 학생 조회 기능(1) - 특정 학생의 번호를 입력하면 그 학생 전체 정보를 반환
    function getStudent(uint _n) public view returns(Student memory){
        return number_Stu[_n-1];
    }
    //* 학생 조회 기능(2) - 특정 학생의 이름을 입력하면 그 학생 전체 정보를 반환
    function getStudent2(string memory _name) public view returns (Student memory){
        return name_Stu[_name];
    }
    // * 학생 점수 조회 기능 - 특정 학생의 이름을 입력하면 그 학생의 점수를 반환
    function getScore(string memory _name) public view returns (uint){
        return name_Stu;[_name].score;
    }
    //* 학생 전체 숫자 조회 기능 - 현재 등록된 학생들의 숫자를 반환
    function getAllstuNum() public view returns (uint){
        return Students.length;
    }
    //* 학생 전체 정보 조회 기능 - 현재 등록된 모든 학생들의 정보를 반환
    function getAllstuInf() public view returns(Student[] memory){
    return Students;
    }

    //* 학생들의 전체 평균 점수 계산 기능 - 학생들의 전체 평균 점수를 반환
    function numAverage() public view returns(uint){
        return 
    }
}